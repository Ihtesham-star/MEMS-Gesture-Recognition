...
def create_virtual_pattern(image, pattern_type="sine", scale=100, color=(0, 255, 255), thickness=2):
    ...
    if pattern_type == "sine":
        ...
        points = []
        start = w//5
        end = int(4.5 * w//5)  # Convert float multiplication to integer
        for x in range(start, end):
            y = int(center_y + scale * math.sin((x - start) * 0.01))
            points.append((x, y))
        ...
        cv2.putText(image, "Sinusoidal Waveform", (w//5, center_y - scale - 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
        cv2.putText(image, "f(x) = A·sin(ωx)", (w//5, center_y - scale - 60),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)

        # Add amplitude markers
        cv2.line(image, (w//6 + 100, center_y), (w//5 + 100, center_y + scale), color, 1)
        cv2.putText(image, f"A={scale}px", (w//5 + 105, center_y + scale//2),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)

    elif pattern_type == "square":
        # Create a square to trace
        side = scale * 2
        top_left = (center_x - side//2, center_y - side//2)

        cv2.rectangle(image, top_left, (top_left[0] + side, top_left[1] + side), color, thickness)

        # Add corner points and midpoints
        corners = [
            (top_left[0], top_left[1]),  # Top-left
            (top_left[0] + side, top_left[1]),  # Top-right
            (top_left[0] + side, top_left[1] + side),  # Bottom-right
            (top_left[0], top_left[1] + side)   # Bottom-left
        ]

        for corner in corners:
            cv2.circle(image, corner, 5, color, -1)
            key_points.append(corner)

        # Label
        cv2.putText(image, f"Square: {side}x{side}px", (top_left[0], top_left[1] - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
        ...
        # Add key points around the circle
        ...
        for formula in formulas:
            ...
            text_size = cv2.getTextSize(formula, cv2.FONT_HERSHEY_SIMPLEX, 1.5, thickness)[0]
            text_x = center_x - text_size[0] // 2

            # Draw the formula
            cv2.putText(image, formula, (text_x, y_offset),
    ...
    return image, key_points, path_points  # Add path_points to the return value
...
def magnetic_path_following(current_pos, path_points, attraction_strength=0.5, max_distance=30):
    """
    Apply a magnetic attraction to the cursor when near the path

    Parameters:
    - current_pos: Current finger position (x, y)
    - path_points: List of points defining the path
    - attraction_strength: How strongly to pull toward the path (0-1)
...
# Magnetic path following parameters
attraction_strength = 0.6  # Initial attraction strength
...
pattern_scale = 400
pattern_thickness = 12  # New variable for thickness
...
try:
    with dai.Device(pipeline) as device:
        ...
        while True:
            ...
            image, key_points, path_points = create_virtual_pattern(image, pattern_type=current_pattern,
            ...
            if results.multi_hand_landmarks:
                ...
                # Apply magnetic path following
                if path_points:  # Make sure we have path points
                    # Apply increasing attraction strength when closer to the path
                    attraction_strength = 0.6  # Default strength

                    # Find distance to nearest path point for visualization
                    min_path_distance = float('inf')
                    for point in path_points:
                        dist = math.sqrt((smooth_x - point[0])**2 + (smooth_y - point[1])**2)
                        if dist < min_path_distance:
                            min_path_distance = dist

                    # Dynamically adjust attraction strength based on distance
                    if min_path_distance < 50:
                        # Stronger attraction when closer to path
                        attraction_strength = 0.7 - (min_path_distance / 100)

                    # Apply magnetic path following
                    smooth_x, smooth_y = magnetic_path_following(
                        (smooth_x, smooth_y),
                        path_points,
                        attraction_strength=attraction_strength,
                        max_distance=50  # Attraction radius
                    )

                    # Display current magnetic attraction status
                    attraction_text = f"Attraction: {int(attraction_strength * 100)}%"
                    cv2.putText(image, attraction_text, (10, 210), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 165, 255), 2)
                ...
                # Draw line showing magnetic attraction (if within range)
                if path_points:
                    closest_path_point = None
                    min_distance = float('inf')
                    for point in path_points:
                        distance = math.sqrt((smooth_x - point[0])**2 + (smooth_y - point[1])**2)
                        if distance < min_distance:
                            min_distance = distance
                            closest_path_point = point

                    # Draw line to closest path point if within attraction range
                    if closest_path_point and min_distance <= 50:
                        cv2.line(image, (smooth_x, smooth_y), closest_path_point, (0, 165, 255), 2)
                        cv2.circle(image, closest_path_point, 5, (0, 165, 255), -1)
            ...
            if key == 27:  # ESC key
                ...
                attraction_strength = min(0.9, attraction_strength + 0.1)
                print(f"Increased magnetic attraction to {attraction_strength:.1f}")
            elif key == ord('n'):
                attraction_strength = max(0.1, attraction_strength - 0.1)
                print(f"Decreased magnetic attraction to {attraction_strength:.1f}")
